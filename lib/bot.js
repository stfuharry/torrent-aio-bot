...
/search 1337x {query} - SEARCH
/details 1337x {link} - DETAILS
/download {magnet link} - DOWNLOAD
/status {magnet link} - STATUS
/remove {magnet link} - REMOVE
/status - STATUS
...
function bot(torrent, bot) {
  ...
  bot.onText(searchRegex, async (msg, match) => {
    ...
    bot.sendMessage(from, "Life without you is like a broken pencil…pointless. Searching..");

    const data = await axios(`${api}api/v1/search/http://1337x.am/?query=${query}`).then(({ data }) => data);
  ...
  bot.onText(detailsRegex, async (msg, match) => {
    ...
    const data = await axios(`${api}/details/$http://1337x.am/?query=${query}`).then(({ data }) => data);
    if (!data || data.error) {
  ...
  bot.onText(removeRegex, (msg, match) => {
    ...
    try {
      torrent.remove(link);
      bot.sendMessage(from, "SuccessFully Removed");
    } catch (e) {
